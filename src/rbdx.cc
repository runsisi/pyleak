...
//class Map : public memory::map<librbdx::K, librbdx::V, librbdx::RawAllocator> {
//public:
//    static void *operator new(size_t s) {
//        using namespace memory::literals;
//        memory::memory_pool<> pool(memory::map_node_size<std::pair<librbdx::K, librbdx::V>>::value, 4_KiB);
//        return ::new memory::map<librbdx::K, librbdx::V, librbdx::RawAllocator>{std::move(pool)};
//    }
//};
...
using Map_string_2_pair_image_info_t_int = librbdx::Map;
...
//using namespace memory::literals;
//std::shared_ptr<memory::memory_pool<>> pool =
//    std::make_shared<memory::memory_pool<>>(memory::map_node_size<std::pair<K, V>>::value, 4_KiB);
...
////      pool = std::make_shared<memory::memory_pool<>>(memory::map_node_size<std::pair<K, V>>::value, 4_KiB);
//
//      std::cerr << "-------------- new ------------\n";
////      return ::new Map_string_2_pair_image_info_t_int;
//      return ::new memory::map<librbdx::K, librbdx::V, memory::memory_pool<>>(*pool);
//    }));

    b.def("__enter__", [](Map_string_2_pair_image_info_t_int& self) {
      std::cerr << "-------------- __enter__  ------------\n";
      return self;
    });

    // pybind11: how to implement a with context manager
    // https://stackoverflow.com/questions/54025245/pybind11-how-to-implement-a-with-context-manager
    b.def("__exit__", [](Map_string_2_pair_image_info_t_int& self,
...
//    cls.def(py::init<>());
    ...
    cls.def(py::init([]() {
      //      using namespace memory::literals;
      //      memory::memory_pool<memory::node_pool, memory::default_allocator> pool(memory::map_node_size<std::pair<K, V>>::value, 4_KiB);
      //      memory::map<K, V, decltype(pool)> infos(pool);
            std::cerr << "-------------- new ------------\n";
            return ::new size_info_t;
      //      return ::new memory::map<librbdx::K, librbdx::V, librbdx::RawAllocator>(pool);
          }));
