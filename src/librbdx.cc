...
//  memory::map<xK, xV, xRawAllocator> tinfos(pool);
  ...
  using xMap = std::map<xK, xV, std::less<xK>, boost::fast_pool_allocator<std::pair<const xK, xV>>>;
  using xPool = boost::singleton_pool<boost::fast_pool_allocator_tag, sizeof(xMap::value_type)>;
  xMap tinfos;
